...
dat <- dat %>%
  ...
  mutate(aBreastFeed_full_stopped = as.numeric(if_else( 
  ...
  mutate(aBreastFeed_total_stopped = 
...
imp_model <- gam(aBreastFeed_total_stopped ~ aAGE, data=mod_dat,
...
# to drink a breast milk. For mixed models, quantile regression, and elastic net analysis, we will
